...
func (kv *KVServer) Get(args *GetArgs, reply *GetReply) {
	...
	ok, resultOp := kv.processRequest(&op)

	reply.Err = resultOp.Err
	reply.Value = op.Value
}

func (kv *KVServer) processRequest(op Op) (bool, Op) {
	ok, index := kv.insertRequstInPool(op)
	if !ok {
		return false, kv.getErrorOp()
	}
}

func (kv *KVServer) getErrorOp() {
	return Op {
		Err: WrongLeader
		Value: ""
	}
...
func (kv *KVServer) insertRequstInPool(op Op) (bool, int) {
