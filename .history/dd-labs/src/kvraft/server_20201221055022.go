...
func (kv *KVServer) isOriginRequest(op Op) {
...
func (kv *KVServer) resetResultChannelOf(requestID int64) {
	kv.mu.Lock()
	defer kv.mu.Unlock()

	if _, ok = kv.resultOf[requestID]; !ok {
		kv.resultOf[requestID] = make(chan Op, 1)
	} else {
		kv.drainResultDataOf(requestID)
	}
}

func (kv *KVServer) drainResultDataOf(requestID int64) {
