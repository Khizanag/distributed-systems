...
// 			fmt.Printf("				Follower  Info: logLen: %v	lastLogTerm: %v	lastLogIndex: %v\n", len(r.logs), r.getLastLog().Term, r.getLastLog().Index)
// 			fmt.Printf("				Candidate Info: logLen: %v	lastLogTerm: %v	lastLogIndex: %v\n", -1, args.LastLogTerm, args.LastLogIndex)
// 		}
// 		r.acceptVoteRequest(args, reply)
// 	} else {
// 		if isDebugMode {
...
// // 			fmt.Printf("-- RequestVote: Raft #%v voted for Raft #%v(CustomVote)\n", r.me, args.CandidateID)
// // 		}
// // 		r.acceptRequestVote(args, reply)
// // 	} else if !r.candidateLogIsUpToDate(args.LastLogIndex, args.LastLogTerm) {
// // 		if isDebugMode {
// // 			fmt.Printf("-- RequestVote: Raft #%v rejected Raft #%v: Candidate log is not up to date\n", r.me, args.CandidateID)
// // 		}
// // 		r.rejectRequestVote(args, reply)
...
// // func (r *Raft) acceptRequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
// // 	reply.VoteGranted = true
...
// // 			"			Raft #%v [currentTerm: %v		logLen: %v	lastLog.Term: %v		lastLog.Index: %v] rejected\n"+
// // 			"			Raft #%v [currentTerm: %v		logLen: %v	lastLog.Term: %v		lastLog.Index: %v]\n",
// // 			r.me, r.currentTerm, len(r.logs), r.getLastLog().Term, r.getLastLog().Index,
// // 			args.CandidateID, args.Term, -1, args.LastLogTerm, args.LastLogIndex)
// // 	}
// // 	reply.VoteGranted = false
// // }
...
// 	r.updateElectionRequestTimeSafe()
...
// 		if r.killed() || r.getRoleSafe() != Leader {
// 			break
// 		}
...
func (r *Raft) readPersist(data []byte) {
	...
	if d.Decode(&currentTerm) != nil ||
		...
		d.Decode(&logs) != nil ||
		d.Decode(&dead) != nil {
		fmt.Printf("-- readPersist: error during reading persist memory\n")
	} else {
...
func (r *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if args.Term < r.currentTerm {
		...
		if isDebugMode {
			fmt.Printf("-- Raft #%v Didn't Appended Entries from %v -> Reason: len(r.logs) <= args.PrevLogIndex -- %v <= %v\n", r.me, args.LeaderID, len(r.logs), args.PrevLogIndex)
		}
		...
		// Reply false if log doesn't contain an entry at prevLogIndex whose term matches prevLogTerm (§5.3)
		if isDebugMode {
			fmt.Printf("-- Raft #%v Didn't Appended Entries from %v -> Reason: r.logs[args.PrevLogIndex].Term(%v) != args.PrevLogTerm(%v)\n", r.me, args.LeaderID, r.logs[args.PrevLogIndex].Term, args.PrevLogTerm)
		}
		r.rejectAppendEntriesRequest(args, reply)
...
func (r *Raft) acceptAppendEntriesRequest(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	r.tryUpdateCurrentTermAndBecomeFollower(args.Term)
	r.role = Follower
	r.updateElectionRequestTime()
}
...
func (r *Raft) tryUpdateCurrentTermAndBecomeFollower(newTerm int) {
...
func (r *Raft) rejectRequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	// oldElectionTime := r.electionRequestTime
	// r.tryUpdateCurrentTermAndBecomeFollower(args.Term)
	// r.electionRequestTime = oldElectionTime
}
...
func (r *Raft) monitorCommits() {
	for !r.killed() {
		...
		for r.commitIndex > r.lastApplied {
			...
			if isDebugMode {
				fmt.Printf("-- -- -- -- -- monitorCommits: Raft #%d committed %v on index: %v\n", r.me, applyMsg.Command, applyMsg.CommandIndex)
			}
			r.applyCh <- applyMsg
		}
		r.mu.Unlock()
...
func (r *Raft) startElections() {
	r.mu.Lock()
	if isDebugMode {
		fmt.Printf("-- Raft #%d started Elections with: currentTerm: %v		commitIndex: %v		logLen: %v\n", r.me, r.currentTerm+1, r.commitIndex, len(r.logs))
	}
	r.role = Candidate
	r.currentTerm++
	r.persist()
	...
	for i := range r.peers {
		...
		go func(index int) {
			...
			if reply.VoteGranted {
				...
				r.mu.Lock()
				r.tryUpdateCurrentTermAndBecomeFollower(reply.Term)
				r.mu.Unlock()
			}
		}(i)
	}
	...
	for { // wait for other servers
		if r.getRoleSafe() != Candidate {
			...
			r.mu.Unlock()
			go r.broadcastHeartBeats() // TODO CHECK
			break
		} else if atomic.LoadInt32(&numRejects) >= numServers/2 { // wasn't chosen
			break
		} else if time.Now().After(r.getElectionRequestTimeSafe()) { // time is overs
			break
		} else {
		...
		go func(index int) {
			...
			for {
				...
				args := AppendEntriesArgs{
					Term:         savedTerm,
					LeaderID:     r.me,
					PrevLogIndex: r.logs[logIndex].Index,
					PrevLogTerm:  r.logs[logIndex].Term,
					Entries:      r.logs[r.nextIndex[index]:],
					LeaderCommit: r.commitIndex,
				}
				...
				if status {
					if reply.Success {
						...
						if reply.Term > savedTerm {
							if isDebugMode {
								fmt.Printf("-- Raft #%v became FOLLOWER while trying to broadcast heartbeats to %v\n", r.me, index)
							}
							r.currentTerm = reply.Term
							r.role = Follower
							r.votedFor = -1
							...
							r.nextIndex[index] = reply.MismatchIndex
							if reply.MismatchIndex <= 0 && isDebugMode {
								fmt.Printf("`````````` -- broadcastHeartbeats: reply.MismatchIndex = %v\n", reply.MismatchIndex)
							}
						}
					}
				}
...
func (r *Raft) tryBecomingCandidate() {
	if time.Now().After(r.getElectionRequestTimeSafe()) { // heartbeat should already be received
		r.startElections()
	}
}
...
func (r *Raft) updateElectionRequestTime() {
	if isDebugMode {
		fmt.Printf("........................................ Raft #%v updated election request time\n", r.me)
	}
	numMilliSeconds := rand.Intn(MaxWaitMSsForRequest-MinWaitMSsForRequest) + MinWaitMSsForRequest
	r.electionRequestTime = time.Now().Add(time.Duration(numMilliSeconds) * time.Millisecond)
}

func (r *Raft) getElectionRequestTimeSafe() time.Time {
	r.mu.Lock()
	defer r.mu.Unlock()
	return r.electionRequestTime
}
