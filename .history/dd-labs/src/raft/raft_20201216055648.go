...
func (r *Raft) sendRequestVote(index int, args *RequestVoteArgs, reply *RequestVoteReply, numAccepts *int32, numRejects *int32) {
...
func (r *Raft) startElections() {
	...
	for i := range r.peers {
		...
		go r.sendRequestVote(index, &args, &reply, &numAccepts, &numRejects)
		go func(index int) {
			if r.killed() || r.getRoleSafe() != Candidate {
				return
			}
