...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) { // TODO CHECKKK
	// r.mu.Lock()
	// defer r.mu.Unlock()
	// defer r.persist()
	...
	if args.Term < rf.currentTerm {
		...
		reply.MismatchIndex = rf.getLastLog().Index + 1
		return
	}
	...
	if args.PrevLogIndex >= baseIndex && args.PrevLogTerm != rf.logs[args.PrevLogIndex-baseIndex].Term {
		...
		rf.logs = rf.logs[:args.PrevLogIndex-baseIndex+1]
		rf.logs = append(rf.logs, args.Entries...)

		reply.Success = true
		reply.MismatchIndex = args.PrevLogIndex + len(args.Entries)
...
func min(a int, b int) int { // TODO remove
	if a < b {
		return a
	} else {
		return b
...
func (r *Raft) sendAppendEntriesHandler(index int) {
	// TODO do this in for lloop
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	if status == false || r.role != Leader || r.currentTerm != args.Term {
		// r.mu.Unlock()
		return
	}

	if r.currentTerm < reply.Term {
		r.updateTermAndBecameFollower(reply.Term)
		return
	}

	if reply.Success { // TODO CHECKKK
		r.matchIndex[index] = r.getLastLog().Index
		r.nextIndex[index] = r.matchIndex[index] + 1
	} else {
		r.updateCommitIndexes(false)
	}
}

func (r *Raft) updateTermAndBecameFollower(newTerm int) {
...
//
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) { // TODO check for
...
//
func (rf *Raft) isUpToDate(candidateTerm int, candidateIndex int) bool { // TODO delete
	term, index := rf.getLastLog().Term, rf.getLastLog().Index
...
func (r *Raft) sendRequestVoteHandler(index int, newTerm int) {
	...
	if status == false || r.role != Candidate || r.currentTerm != args.Term {
		return
	}
	...
	if reply.VoteGranted {
		// atomic.AddInt32(acceptedVoteRequestCount, 1)
		r.voteCount++
		// var numServers int32 = int32(r.getServersCount())
		// if atomic.LoadInt32(acceptedVoteRequestCount) > numServers/2 {
		if r.voteCount > r.getServersCount()/2 {
			r.role = Leader
			r.persist() // TODO remove
			if isDebugMode {
			...
			r.becameLeaderCh <- true
		}
	}
}

// TODO add wasNotSelectedAsLeaderCh
...
func (r *Raft) startElections() {
	...
	r.currentTerm++
	var newTerm = r.currentTerm
	// var acceptedVoteRequestCount int32 = 1 // self vote
	r.votedFor = r.me
	r.voteCount = 1
	r.persist()
	r.mu.Unlock()

	// TODO create args outside the for
	...
	for i := range r.peers {
		...
		go r.sendRequestVoteHandler(i, newTerm)
	}
}
...
func (r *Raft) broadcastHeartBeats() {
	...
	for i := range r.peers {
		...
		go r.sendAppendEntriesHandler(i)
	}
}
...
func (r *Raft) watcher() {
	for !r.killed() {
		...
		case Leader:
			...
			time.Sleep(60 * time.Millisecond) // TODO change time sleep K
		case Candidate:
			go r.startElections()
			...
			case <-r.heartbeatReceivedCh:
				r.role = Follower
			case <-r.becameLeaderCh:
			case <-time.After(time.Millisecond * time.Duration(rand.Intn(300)+200)): // TIMEOUT, do nothing
			}
		case Follower:
			select {
