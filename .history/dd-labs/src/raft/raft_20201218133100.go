...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if args.PrevLogTerm != rf.log[args.PrevLogIndex].Term {
		...
		rf.receiveAppendEntriesRequest()
	}
}

func (r *Raft) receiveAppendEntriesRequest() {
	r.log = rf.log[:args.PrevLogIndex+1]
	r.log = append(rf.log, args.Entries...)

	reply.Success = true
	reply.NextTryIndex = args.PrevLogIndex + len(args.Entries)

	if r.commitIndex < args.LeaderCommit {
		r.commitIndex = min(args.LeaderCommit, rf.getLastLogEntry(false).Index)
	}
}
