...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	hasOldTerm := args.LastLogTerm < rf.logs[len(rf.logs)-1].Term
	hasOldIdx := (args.LastLogTerm == rf.logs[len(rf.logs)-1].Term) && args.LastLogIndex < len(rf.logs)-1
...
func (r *Raft) startElections() {
	...
	for { // wait for other servers
		...
		if atomic.LoadInt32(&numAccepts) > numServers/2 { // became leader
			...
			time.Sleep(SleepMSsForCandidateDuringElections * time.Millisecond)
		}
	}
}
