...
func (r *Raft) trimLog(lastIncludedIndex int, lastIncludedTerm int) {
	newLog := make([]LogEntry, 0)
	zerothLogEntry := LogEntry{
		Index: lastIncludedIndex,
		Term:  lastIncludedTerm,
	}
	newLog = append(newLog, zerothLogEntry)

	for i := len(r.log) - 1; i >= 0; i-- {
		if r.log[i].Index == lastIncludedIndex && rf.log[i].Term == lastIncludedTerm {
